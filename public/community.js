(() => {
  ...
  const renderStats = async () => {
    try {
      ...
      setLiveStatus('Start npm start to connect the real forum');
    }
  };
  ...
  const renderCategories = () => {
    ...
    state.categories.forEach((cat) => {
      ...
      button.innerHTML = `<span class="category-icon">${Aion.escapeHtml(cat.icon || '✦')}</span><span>${Aion.escapeHtml(cat.name)}</span><small>${cat.thread_count || 0}</small>`;
    ...
    if (topicCategory) {
      topicCategory.innerHTML = state.categories.map((cat) => `<option value="${Aion.escapeHtml(cat.slug)}">${Aion.escapeHtml(cat.name)}</option>`).join('');
    }
  };

  const sortThreads = () => {
    const time = (t) => new Date(t.last_activity_at || t.created_at || 0).getTime() || 0;
    if (state.sort === 'top') state.threads.sort((a, b) => Number(b.is_pinned) - Number(a.is_pinned) || Number(b.votes || 0) - Number(a.votes || 0) || time(b) - time(a));
    else if (state.sort === 'new') state.threads.sort((a, b) => new Date(b.created_at || 0) - new Date(a.created_at || 0));
    else if (state.sort === 'views') state.threads.sort((a, b) => Number(b.views || 0) - Number(a.views || 0) || time(b) - time(a));
    else state.threads.sort((a, b) => Number(b.is_pinned) - Number(a.is_pinned) || time(b) - time(a));
  };

  const matchesSearch = (thread) => {
    const q = state.q.trim().toLowerCase();
    if (!q) return true;
    const tags = Array.isArray(thread.tags) ? thread.tags.join(' ') : String(thread.tags || '');
  ...
  const renderThreads = () => {
    ...
    feed.innerHTML = '';
    if (!list.length) {
      feed.innerHTML = `<article class="thread-card glass-card"><div><p class="eyebrow">No topics yet</p><h2>Start the first real discussion.</h2><p>Every topic created here is stored in the database and broadcast live to connected visitors.</p></div></article>`;
      return;
    }
    ...
    list.forEach((thread) => {
      ...
      node.querySelector('.category-pill').textContent = thread.category_name || thread.category || thread.category_slug || 'General';
      const status = node.querySelector('.thread-status');
      status.textContent = thread.status === 'solved' ? 'Solved' : Number(thread.is_pinned) ? 'Pinned' : thread.type || 'Open';
      status.classList.add(thread.status === 'solved' ? 'solved' : Number(thread.is_pinned) ? 'pinned' : '');
      node.querySelector('h2').textContent = thread.title;
      node.querySelector('p').textContent = thread.body;
      const tags = Array.isArray(thread.tags) ? thread.tags : String(thread.tags || '').split(',').filter(Boolean);
      node.querySelector('.thread-tags').innerHTML = tags.slice(0, 4).map((tag) => `<span>#${Aion.escapeHtml(tag.trim())}</span>`).join('');
      ...
      node.querySelector('.thread-author').textContent = `Started by ${thread.display_name || thread.username || 'AION member'} · last active ${Aion.formatDate(thread.last_activity_at || thread.created_at)}`;
      feed.append(node);
    });
  };
  ...
  const renderTags = () => {
    ...
    state.threads.forEach((thread) => {
      const list = Array.isArray(thread.tags) ? thread.tags : String(thread.tags || '').split(',');
      list.forEach((tag) => {
        const clean = tag.trim();
        if (clean) tags.set(clean, (tags.get(clean) || 0) + 1);
      });
    });
    const html = [...tags.entries()].sort((a,b) => b[1] - a[1]).slice(0, 12).map(([tag]) => `<span>#${Aion.escapeHtml(tag)}</span>`).join('');
  ...
  const loadCategories = async () => {
    try {
      ...
      categoryBox.innerHTML = `<button class="category-row active" data-category="all"><span class="category-icon">!</span><span>Forum offline</span><small>API</small></button>`;
    }
  };

  const loadThreads = async ({ quiet = false } = {}) => {
    if (!quiet) feed.innerHTML = '<article class="thread-skeleton glass-card"></article><article class="thread-skeleton glass-card"></article><article class="thread-skeleton glass-card"></article>';
    try {
  ...
  const selectCategory = (category) => {
    ...
    state.filter = '';
    document.querySelectorAll('.category-row').forEach((button) => button.classList.toggle('active', button.dataset.category === category));
    document.querySelectorAll('.filter-row').forEach((button) => button.classList.remove('active'));
  ...
  document.querySelector('[data-refresh]')?.addEventListener('click', () => { loadCategories(); loadThreads(); renderStats(); });
  document.querySelectorAll('[data-open-topic]').forEach((button) => button.addEventListener('click', () => topicModal?.showModal()));
  ...
  topicForm?.addEventListener('submit', async (event) => {
    ...
    try {
      const payload = await Aion.api('/api/threads', { method: 'POST', body: JSON.stringify(raw) });
      topicModal.close();
      topicForm.reset();
  ...
  window.addEventListener('aion:auth', () => { loadThreads({ quiet: true }); renderStats(); });
